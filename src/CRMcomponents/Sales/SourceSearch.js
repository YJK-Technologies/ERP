...
const SourceSearch = ({ showSourceSearch, onSourceSearchClose, onSaveSource, selectedColumnId }) => {
    ...
    const columnDefs = [
        { headerName: 'Source Id', field: 'Source_ID', flex: true },
        { headerName: 'Source Name', field: 'SourceName', flex: true },
    ];

    if (!showSourceSearch) return null;
        ...
        try {
            ...
            selectedTags.forEach(tag => {
                ...
                if (parts.length > 1) {
                    ...
                    if (field === "SourceId") payload.Source_ID = value;
                    if (field === "SourceName") payload.SourceName = value;
                }
            });

            const response = await fetch(`${config.apiBaseUrl}/searchSource`, {
                method: 'POST',
                headers: {
    ...
    const handleRowClick = (params) => {
        const selectedSourceId = params.data.Source_ID;
        if (onSaveSource) {
