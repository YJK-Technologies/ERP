...
const NewContactModal = ({
  ...
  if (contactData) {
    ...
    if (info) {
      const type = (info.Company_or_Persona || "company").toLowerCase();
      setType(type);
  ...
  const handleSave = async () => {
    try {
      ...
      if (type === "person") {
        ...
        formData.append("Stage", selectedColumnId);
      ...
      const response = await fetch(
        `${config.apiBaseUrl}/CRM_ContactInfoInsert`,
        {
          method: "POST",
          body: formData,
        }
                ...
                <div>
                  <div className="row row-cols-1 row-cols-md-2 g-3">
                    {(contacts || []).map((c, index) => (
                      ...
                      >
                        <div className="card h-100 p-1 rounded-0">
                ...
                <div>
                  <ul className="nav nav-pills mb-3">
                    <li className="nav-item">
                      <button className="nav-link ">Sales</button>
                    </li>
                    <li className="nav-item">
                      <button className="nav-link">Purchase</button>
                    </li>
                  </ul>
                  <div className="p-2 bg-white border rounded">
                    <p>No sales/purchase records yet.</p>
                  </div>
                </div>
              )}
              {activeTab === "notes" && (
                <div>
                  ...
                  />
                  <button className="btn btn-sm btn-success">Save Note</button>
                </div>
              )}
            </div>
          </div>
