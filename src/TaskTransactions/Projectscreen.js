            ...
            if (selectedRow?.TaskMasterID) {
              ...
              if (taskResponse.ok) {
                ...
                if (taskData.length > 0) {
                  ...
                  setHourseTaken(taskData[0].HourseTaken);
                }
          ...
          if (selectedRow?.TaskMasterID) {
            ...
            if (taskResponse.ok) {
              ...
              if (taskData.length > 0) {
                ...
                setDescription(task.Description);
                setTaskStatus(task.TaskStatus);
                settaskid(task.TaskMasterID);
                setHourseTaken(task.HourseTaken);

                // 🔽 NEW IMPLEMENTATION (same as handleTaskClick)
                if (task.Documents && task.Documents.data) {
                  const byteArray = new Uint8Array(task.Documents.data);
...
const handlesave = async () => {
  ...
  const isTransfer = selectedtstatusSC?.value === "Transfer";

  if (
    !tasktitle ||
    !usercode ||
    (!estimatedhours && !isFeedback && !isTransfer) ||
    !status_typeSC ||
    !taskid ||
    !text ||
    !PriorityLevel
      ...
      <div>
        ...
        <div className="shadow-lg p-2 bg-body-tertiary rounded mt-2">
          ...
          <div class="row ms-3 me-3 mb-3">
            ...
            {selectedtstatusSC?.value !== "Feedback" && selectedtstatusSC?.value !== "Transfer" && (
              <div className="col-md-3 form-group mb-2">
                <div className="exp-form-floating">
