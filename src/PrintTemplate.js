  ...
  const [Academic, setAcademic] = useState([
    {
      relation: "Screens",
      members: [
        { screenName: "", templatename: "", Templates: null, documentUrl: "" },
      ],
    },
  ]);
      ...
      prev.map((item) =>
        item.relation === relation
          ? {
              ...
              members: [
                ...item.members,
                { screenName: "", templatename: "", institution: "" },
              ],
            }
          : item,
      ),
      ...
      Academic.flatMap((relationGroup) =>
        relationGroup.members.map(async (member) => {
          if (member.Templates) {
            ...
            const fileBase64 = member.Templates
              ? await convertToBase64(member.Templates)
              : null;
      ...
      if (response.ok) {
        ...
        if (searchData.length > 0) {
          const updatedTemplateMembers = searchData.reduce((acc, item) => {
            ...
            if (Templates) {
              const { blobUrl, file } = convertBufferToBlobUrlAndFile(
                Templates,
                Template_name,
              );
              documentUrl = blobUrl;
    ...
    <div className="container-fluid Topnav-screen">
      ...
      <div className="shadow-lg p-1 bg-light rounded">
        <div className=" mb-0 d-flex justify-content-between">
          ...
          <div className="d-flex justify-content-end me-4">
            <div onClick={handleSave} className="col-md-2">
              <a
      ...
      <div class="mb-4 mt-2">
        {Academic.map((relationGroup, relationIndex) => (
          <div
            key={relationIndex}
            className="shadow-sm p-1 bg-light rounded-0 "
          >
            {relationGroup.members.map((member, index) => (
              <div key={index} className="row  mt-3">
                ...
                <div className="col-md-2 form-group mb-2">
                  <label
                    className={`${error && !member.screenName ? "red" : ""}`}
                  >
                    Screens
                ...
                <div className="col-md-2 form-group mb-2">
                  <label
                    className={`${error && !member.templatename ? "red" : ""}`}
                  >
                    Template Name{" "}
                    {showAsterisk && <span className="text-danger">*</span>}
                  </label>
