...
export default function PaymentPopup({ open, handleClose, ProductData }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`${config.apiBaseUrl}/GetClient_PaymentDataSearch`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({ Client_code, Payment_Type, Payment,Payment_Date, Company_code: sessionStorage.getItem('selectedCompanyCode') }) // Send company_no and company_name as search criteria
      });
      if (response.ok) {
